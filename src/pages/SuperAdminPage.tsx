...
export default function SuperAdminPage() {
  ...
  useEffect(() => {
    const fetchSystemUsers = async () => {
      try {
        const response = await fetch("https://gazi-group-backend-1.onrender.com/api/users");
  ...
  const changeUserTierNode = async (email: string, targetTier: string) => {
    try {
      const response = await fetch("https://gazi-group-backend-1.onrender.com/api/users/update-tier", {
        method: "POST",
